...
export function StatsSection({ translations: t }: StatsSectionProps) {
  ...
  return (
    <section id="stats" className="relative bg-ensemble-50 py-20 sm:py-28 overflow-hidden">
      {/* Glowing separator line at top */}
      <div className="absolute inset-x-0 top-0 flex items-center justify-center">
        <div className="h-px w-full max-w-3xl bg-gradient-to-r from-transparent via-accent-500/30 to-transparent" />
      </div>
        ...
        >
          {t.stats.map((stat, i) => (
            <div key={i} className="text-center">
              ...
              <div className="mt-2 text-sm font-medium text-ensemble-500 sm:text-base">
                {stat.label}
              </div>
            </div>
          ))}
